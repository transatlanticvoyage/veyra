<?php
/**
 * Veyra — Drip Actions Jar admin screen.
 *
 * Read-only dump of every column and row in wp_veyra_post_drip_actions (the
 * queue behind the "Veyra Post Drip Actions" editor sidebar box), registered at
 * /wp-admin/admin.php?page=veyra_drip_actions_jar under the Veyra Hub 1 menu.
 *
...
if (!defined('VEYRA_DAJ_DEFAULT_LIMIT')) {
    define('VEYRA_DAJ_DEFAULT_LIMIT', 500);
}

/** Full name of the table this jar displays. veyra_pda_table() is defined in
 *  veyra.php; fall back to the literal name if this file is ever loaded alone. */
function veyra_daj_table() {
    if (function_exists('veyra_pda_table')) {
        return veyra_pda_table();
    }
    global $wpdb;
...
// ---------------------------------------------------------------------------
add_action('admin_menu', 'veyra_daj_register_menu', 20);
function veyra_daj_register_menu() {
    add_submenu_page(
        'veyra-hub-1',                  // parent (Veyra Hub 1)
        'Drip Actions Jar',             // page title
        'Drip Actions Jar',             // menu label
        'manage_options',               // capability
        'veyra_drip_actions_jar',       // slug -> ?page=veyra_drip_actions_jar
        'veyra_daj_render_page'         // callback
    );
}

/** Aggressive notice/warning/message suppression on this screen only. */
add_action('in_admin_header', 'veyra_daj_suppress_notices', 1);
function veyra_daj_suppress_notices() {
    if (!isset($_GET['page']) || $_GET['page'] !== 'veyra_drip_actions_jar') {
        return;
    }
...
function veyra_daj_render_page() {
    ...
    // "Conduit" arrival: a row id handed over by the Veyra Post Drip Actions box
    // in the post editor, so the user lands looking straight at the row they
    // clicked from.
    ...
    if ($table_exists) {
        ...
        // Newest first — this is an append-only queue log, so the recent end is
        ...
        if (!$show_all) {
            $sql .= ' LIMIT ' . intval(VEYRA_DAJ_DEFAULT_LIMIT);
        }
    ...
    ?>
    <div class="wrap veyra-daj">
        <h1>Drip Actions Jar</h1>

        <p class="veyra-daj-tablename"><?php echo esc_html($t); ?></p>
        ...
        <?php else: ?>
            ...
            <?php if ($conduit_id): ?>
                <div class="veyra-daj-conduit-banner">
                    <?php if ($conduit_found): ?>
                        <span class="veyra-daj-star">&#9733;</span>
            ...
            <div class="veyra-daj-scroll">
                <table class="wp-list-table widefat striped veyra-daj-table">
                    <thead>
                    <tr>
                    ...
                    <?php else: foreach ($rows as $row):
                        ...
                        $first_col  = true; ?>
                        <tr<?php echo $is_conduit ? ' class="veyra-daj-conduit-row"' : ''; ?>>
                            <?php foreach ($columns as $col): ?>
                                <td>
                                    <?php if ($first_col && $is_conduit): ?>
                                        <span class="veyra-daj-star"
                                              title="You came here from this item in the post editor">&#9733;</span>
                                    <?php endif; ?>
                                    <?php $first_col = false; ?>
                                    <?php if ($row[$col] === null): ?>
                                        <span class="veyra-daj-null">NULL</span>
                                    <?php elseif ($row[$col] === ''): ?>
                                        <span class="veyra-daj-empty">(empty)</span>
                                    <?php else: ?>
    ...
    <style>
    .veyra-daj-tablename{font-family:Menlo,Consolas,monospace;font-size:15px;font-weight:600;
        color:#1d2327;background:#f0f0f1;border-left:4px solid #2271b1;padding:8px 12px;
        display:inline-block;margin:4px 0 14px;}
    .veyra-daj-count{margin:0 0 10px;color:#50575e;}
    .veyra-daj-scroll{overflow-x:auto;max-width:100%;}
    .veyra-daj-table{min-width:100%;width:auto;}
    .veyra-daj-table th{white-space:nowrap;font-family:Menlo,Consolas,monospace;font-size:11px;}
    .veyra-daj-table th strong{font-weight:700;}
    .veyra-daj-table td{font-size:12px;vertical-align:top;max-width:340px;
        overflow-wrap:anywhere;word-break:break-word;}
    .veyra-daj-null{color:#a7aaad;font-style:italic;}
    .veyra-daj-empty{color:#c3c4c7;font-style:italic;}
    /* Conduit arrival: the row the user clicked through from. */
    .veyra-daj-star{color:#d63638;font-size:14px;line-height:1;margin-right:4px;}
    .veyra-daj-conduit-banner{background:#fcf0f1;border-left:4px solid #d63638;
        padding:10px 12px;margin:0 0 12px;max-width:900px;}
    .veyra-daj-table tr.veyra-daj-conduit-row > td{background:#fcf0f1 !important;
